...
const Expense = ({ onEdit }) => {
  ...
  const fetchExpenses = async () => {
    try {
      const response = await fetch('https://expense-sss-default-rtdb.firebaseio.com/UserExpense.json');
      if (!response.ok) {
  ...
  const deleteExpenseHandler = async (expenseId) => {
    try {
      const response = await fetch(`https://expense-sss-default-rtdb.firebaseio.com/UserExpense/${expenseId}.json`, {
        method: 'DELETE'
      });
      if (!response.ok) {
