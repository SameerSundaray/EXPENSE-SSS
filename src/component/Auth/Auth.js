...
const Auth = () => {
    ...
    if (UserDetail.Email.trim().length > 0 &&
      ...
      try {
        const response = await fetch(
          "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyCWK-6BYgWzzK-6jzvxpgkwkWDbdJHAwxc",
          {
            method: "POST",
            body: JSON.stringify({
  ...
  const LogInButtonHandler = async (event) => {
    ...
    try {
      const response = await fetch(
        "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyCWK-6BYgWzzK-6jzvxpgkwkWDbdJHAwxc",
        {
          method: "POST",
          body: JSON.stringify({
            email: UserDetail.Email,
